...
// Create a new directory for the output standard deviation images
File.makeDirectory(output + File.separator + "VAR");
...
function processFile(input, output, file) {
	...
	for (f = 2; f <= frames; f++){
		...
		saveAs("Tiff", output + "/VAR/" + baseName + "_std_f" + f + ".tif");
		rename("STD");
